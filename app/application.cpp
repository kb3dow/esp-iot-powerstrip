...
// ... and/or MQTT host and port
	...
	#define MQTT_HOST "nuc.niknat.net"
	#ifndef ENABLE_SSL
...
void onMessageReceived(String topic, String message)
{
    ...
    if (!strcmp("iot/esp/1/led", topic.c_str()))
    {
        digitalWrite(LED_PIN, strcmp("off",message.c_str()) ? 0 : 1);
    }
}
...
void connectOk()
{
	...
	//Serial.println(WifiStation.getMAC().c_str());

	// Run MQTT client
	startMqttClient();

    // RR NOTE: dont have esp publish, external publisher
